"""
League of Legends Minimap Image Generator - ImageDrawer

This module implements the ImageDrawer class to generate minimap images with various elements and output YOLO format annotation data.

1. Load minimap background, fog, champion icons, and other assets
2. Generate minimap with the following elements:
   - Base minimap + fog overlay
   - Towers
   - Neutral camps
   - Minions
   - Champions (with circle outline)
   - Pings
   - Wards
   - Lines and boxes

3. Output YOLO format bounding box annotation (normalized coordinates: class_id center_x center_y width height)

## Reference
- Reference Blog: https://maknee.github.io/blog/2021/League-ML-Minimap-Detection2/
"""
...
class ImageDrawer:
    ...
    def setup(self):
        ...
        for m, label in self.miscs:
            ...
            m = m.transpose((2, 1, 0))
            # support 96x96 icon
            m = cv2.resize(m, (96, 96), interpolation=cv2.INTER_AREA)
            m = m.transpose((2, 1, 0))
    ...
    def draw_champions(self, minimap, DrawOperation, FogFilterOperation, ally_champions, enemy_champions, allow_overlap=True):
        """
        Draw champions icons to minimap
        
        Args:
            minimap: minimap image
            DrawOperation: drawing operation function
            FogFilterOperation: fog filter operation function
            ally_champions: ally champions list
            enemy_champions: enemy champions list
            allow_overlap: whether to allow icon overlap, default is True (allow overlap)
        
        Returns:
            champion_position_data: champion position data list
        """
        ...
        def AddChampions(minimap, champions):
            ...
            for c, label in champions:
                ...
                # try to place icon, if not allowed to overlap, need to check if it overlaps with existing icons
                max_attempts = 100
                attempt = 0
                placed = False
                
                while attempt < max_attempts and not placed:
                    # ensure icon is completely in the map
                    x = random.randint(0, max(0, w - c_w))
                    y = random.randint(0, max(0, h - c_h))
                    ...
                    if len(champion_position_data) > 0:
                        if random.random() < 0.2:
                            ...
                            # Convert back to original coordinates for clustering
                            other_x_orig = int(other_x / self.minimap_ratio[0])
                            other_y_orig = int(other_y / self.minimap_ratio[1])
                            nearness = c_w
                            bias_x = pow(random.random(), 0.5) * (-1 if random.random() < 0.5 else 1)
                            bias_y = pow(random.random(), 0.5) * (-1 if random.random() < 0.5 else 1)
                            x = int(other_x_orig + nearness * bias_x)
                            y = int(other_y_orig + nearness * bias_y)
                            # ensure icon is completely in the map
                            x = max(0, min(x, max(0, w - c_w)))
                            y = max(0, min(y, max(0, h - c_h)))
                    
                    # if not allowed to overlap, check if it overlaps with existing icons
                    if not allow_overlap:
                        overlap_found = False
                        for existing_label, existing_final_x, existing_final_y, existing_final_c_w, existing_final_c_h in champion_position_data:
                            # convert existing position back to original coordinates
                            existing_x_orig = int(existing_final_x / self.minimap_ratio[0])
                            existing_y_orig = int(existing_final_y / self.minimap_ratio[1])
                            existing_c_w_orig = int(existing_final_c_w / self.minimap_ratio[0])
                            existing_c_h_orig = int(existing_final_c_h / self.minimap_ratio[1])
                            
                            # check if it overlaps with existing icons
                            if check_overlap(x, y, c_w, c_h, existing_x_orig, existing_y_orig, existing_c_w_orig, existing_c_h_orig):
                                overlap_found = True
                                break
                ...
                # if tried max_attempts times still cannot place (not allowed to overlap), use the last attempt position
                c_w, c_h = c.shape[1:3]
 
                final_x = int(x * self.minimap_ratio[0])
                final_y = int(y * self.minimap_ratio[1])
                final_c_w = int(c_w * self.minimap_ratio[0])
                final_c_h = int(c_h * self.minimap_ratio[1])
                state = (label, final_x, final_y, final_c_w, final_c_h)
                DrawOperation(c, x, y)
    ...
    def draw_pings(self, minimap, DrawOperation, FogFilterOperation, champion_position_data=None):
        ...
        for i in range(num_pings):
            ...
            ww_h = int(ww / 2)
            circle_x = x - ww_h + int(ping_w)
            circle_y = y - ww_h + int(ping_h)
    ...
    def convert_champion_position_data_to_yolo_format(self, champion_position_data, index, image_width, image_height):
        """
        Convert champion position data to YOLO format
        YOLO format: class_id center_x center_y width height (all values are normalized to 0-1)
        
        Args:
            champion_position_data: (label, x, y, width, height) in final image coordinates
            index: image index
            image_width: final image width
            image_height: final image height
        
        Returns:
            dict: contains YOLO format annotation data
                - annotations: list, each element is "class_id center_x center_y width height" string
                - annotations_list: list, each element is [class_id, center_x, center_y, width, height] list
                - image_id: image ID
        """
        annotations = []
        annotations_list = []
        ...
        for label, x, y, c_w, c_h in champion_position_data:
            ...
            center_x = (x + c_w / 2.0) / image_width
            center_y = (y + c_h / 2.0) / image_height
            
            # Normalize width and height
            width = c_w / image_width
            height = c_h / image_height
            
            # Get class ID
            champion_id = self.champion_to_id[label]
            class_id = champion_id - 1
            ...
            annotation_str = f"{class_id} {center_x:.6f} {center_y:.6f} {width:.6f} {height:.6f}"
            annotations.append(annotation_str)
            
            annotations_list.append([class_id, center_x, center_y, width, height])
    ...
    # Main function
    def generate_data(self, index, allow_overlap=True):
        """
        Generate a minimap image and corresponding YOLO format annotation
        
        Args:
            index: image index
            allow_overlap: whether to allow champion icon overlap, default is True (allow overlap)
        
        Returns:
            tuple: (final_image, yolo_data)
                - final_image: final generated image (torch tensor)
                - yolo_data: YOLO format annotation data (dictionary)
        """
        ...
        champion_position_data = self.draw_champions(minimap, DrawOperation, FogFilterOperation, ally_champions, enemy_champions, allow_overlap=allow_overlap)
        ...
        _, image_height, image_width = final_image.shape
        
        data = self.convert_champion_position_data_to_yolo_format(
            champion_position_data, index, image_width, image_height
        )
